...
$events_html.='<div id="event-'. $event_id .'"'.$cat_colors_attr.' class="ect-cover-event '.$style.' '.$event_type.'" itemscope itemtype="http://schema.org/Event">
                ...
                $events_html.='<div class="ect-cover-date">
                    ...
                    if ( tribe_get_map_link($event_id) ) {
						$events_html.='<span class="ect-google-map">'.tribe_get_map_link_html($event_id).'</span>';
					}
                    ...
                    $events_html.= '<div class="ect-cover-readmore"><a href="'.tribe_get_event_link($event_id).'" title="'.get_the_title($event_id) .'" rel="bookmark">'.__('Find out more','the-events-calendar').'</a></div>';
                    if($socialshare=="yes") {
