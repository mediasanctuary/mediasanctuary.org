...
if ($event_all_day) {
    ...
    if ($start_date != 'All day') {
        if (strpos($ev_time, '-') !== false) {
            $end_date = explode(' - ', $ev_time)[1];
        } else {
            $end_date = '';
        }
    }
...
if (is_array($terms2)) {
    foreach ($terms2 as $term) {
        array_push($tagList, esc_html($term->name));
    }
}
...
$events_html .= '<td class="ect-advance-list-tittle-name "><a href="' . esc_url(tribe_get_event_link($event_id)) . '" target="_blank" >' . esc_html($tittle) . '</a></td>';
...
if (!empty($end_date)) {
    $events_html .= '<td class="ect-advance-list-time" data-search="' . esc_attr(str_replace(' ', '', $ev_time)) . '"><div class="ect-event-time"><span class="ect-str-date">' . esc_html($start_date) . '</span>- <span class="end-date">' . esc_html($end_date) . '</span></div></td>';
} else {
    $events_html .= '<td class="ect-advance-list-time" data-search="' . esc_attr(str_replace(' ', '', $ev_time)) . '"><div class="ect-event-time"><span class="ect-str-date">' . esc_html($start_date) . '</span></div></td>';
...
$events_html .= '<td class="ect-cattag-hide">' . esc_html($tagValues) . '</td>';
if ($hide_venue === 'no') {
