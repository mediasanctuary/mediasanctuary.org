...
class ect_calendar_template
{
    ...
    |---------------------------------------------|
     */
    public function ect_register_script()
    {
    ...
    public function ect_calendar_shortcode($attr, $content = null)
    {
        ...
        $cal = '<div id="ect-calendar-wrapper" class="ect-custom-calendar" data-date-format="' . esc_attr($date_format) . '" data-time-format="' . esc_attr($time_format) . '" data-skin-color="' . esc_attr($main_skin_color) . '" data-alt-skin-color="' . esc_attr($alternate_color) . '" data-events-limit="' . esc_attr($evt_limit) . '" data-calendar-id="' . esc_attr($ID) . '" data-days-name=["' . implode('","', array_map('esc_attr', $nameOfDays)) . '"] data-featured-bgcolor="' . esc_attr($featured_bgcolor) . '" data-current-lang="' . esc_attr(get_bloginfo('language')) . '" data-featured-textcolor="' . esc_attr($featured_textColor) . '" data-readmore-text="' . esc_attr($readMore) . '">
		<div class="ect-calendar-menu">';
        if ($cat_filter == 'true') {
            $cal .= '<div class="ect-calendar-cat-filter-wrapper" data-calendar-id="' . esc_attr($ID) . '"></div>';
        }
        $cal .= '<span class="ect_renderRange" data-calendar-id="' . esc_attr($ID) . '" class="render-range" data-months=["' . implode('","', array_map('esc_attr', $monthsName)) . '"]></span>
	  </div>
	  <div class="ect-calendar-container">
	  <div class="ect_calendar_events_spinner"><img alt="Loading" src="' . esc_url(ECT_PRO_PLUGIN_URL . 'assets/images/ect-preloader.gif') . '"></div>
	  <div id="ect_calendar-' . esc_attr($ID) . '">
	  </div>
	  </div>
      </div>';
        return $cal;
    ...
    |---------------------------------------------------------------------------------------|
     */
    public function ect_filter_events_rest_data($data, $event)
    {
        $event_id = $data['id'];
    ...
    public function event_category_add_style($term)
    {
		...
		<div class="form-field">
			...
			<?php wp_nonce_field('save_event_category_style', 'event_category_style_nonce');?>
		</div>
			<?php
} else {
    ...
    public function event_save_category_style($term_id)
    {

        if (!isset($_POST['event_category_style_nonce']) || !wp_verify_nonce($_POST['event_category_style_nonce'], 'save_event_category_style')) {
            return;
        }
    ...
    public function event_cat_colorpicker_enqueue($taxonomy)
    {
    ...
    public function event_cat_colorpicker_init()
    {
