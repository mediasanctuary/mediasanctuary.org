...
final class EventsCalendarTemplatesPro 
{
	...
	public function ect_load_files() {
		...
		if( is_admin() ){
			...
			require_once ECT_PRO_PLUGIN_DIR . 'admin/notices/plugin-upgrade-notice.php';
		...
		public  function onInit(){
				// if(version_compare(get_option('ect-v'),'2.3', '<')){		
						ect_pro_create_admin_notice( array(
							'id'=>'ect-pro-setting-migration',
							'message'=>__('<strong>Important Update</strong>:- <strong>Events Shortcodes (Pro) - The Events Calendar Addon</strong> plugin has integrated new settings panel. Please save your settings and check events views.','ect'),
							'review_interval'=>0,  
						) );
				// }
		...
		public function Install_ECT_Notice(){
			if ( current_user_can( 'activate_plugins' ) ) {
				...
				echo '<div class="error CTEC_Msz"><p>' . sprintf( __( 'In order to use <strong>Events Shortcodes (Pro) - The Events Calendar Addon</strong>, Please first install the latest version of <a href="%s" class="thickbox" title="%s">%s</a> and add an event.', 'ect' ), esc_url( $url ), esc_attr( $title ),esc_attr( $title ) ) . '</p></div>';
			}
		}
		...
		public function Install_Ext_Events_Notice() {
			if ( current_user_can( 'activate_plugins' ) ) {
				if( get_transient( 'ect-status-timing' ) ){
					...
					<p><?php echo  sprintf( __( 'In order to set the event status to Online event,Canceled or Postponed, Please first install the latest version of <a href="%s" target="_blank" title="%s">%s</a>.', 'ect' ), esc_url( $url ), esc_attr( $title ),esc_attr( $title ) );?> || <a href="<?php echo $dont_disturb_url ?> "class="ect-review-done ">Not Interested!</a>
					</p>
					</div>
					<?php
