...
$file   ='the-events-calendar-templates-and-shortcode.php';
$folder = basename( ECT_PRO_PLUGIN_DIR );
$hook = "in_plugin_update_message-{$folder}/{$file}";
add_action( $hook, 'ect_pro_plugin_update_message', 10, 2 ); // 10:priority, 2:arguments #
 ...
 */
function ect_pro_plugin_update_message( $args, $response ) {
	
	$new_version    = $response->new_version;
	$upgrade_notice = ect_pro_get_upgrade_notice( $new_version );
 ...
 * @return string
 */
function ect_pro_get_upgrade_notice( $version ) {
	...
	//	$response = wp_safe_remote_get( 'http://feedback.coolplugins.net/wp-content/uploads/2021/09/readme.txt' );
		//} else {
		$response = wp_safe_remote_get( 'https://bitbucket.org/!api/2.0/snippets/CoolHappy/gBBrKE/c60dfe6c46604d549629fcab0925d4d653f8c3bf/files/readme.txt' );
		//}
		if ( ! is_wp_error( $response ) && ! empty( $response['body'] ) ) {
			$upgrade_notice = ect_pro_parse_update_notice( $response['body'], $version );
 ...
 * @return string
 */
function ect_pro_parse_update_notice( $content, $new_version ) {
