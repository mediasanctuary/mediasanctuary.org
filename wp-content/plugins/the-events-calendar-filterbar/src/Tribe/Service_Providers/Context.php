	...
	public function filter_context_locations( array $locations = [] ) {
		...
		$get_fb_additional_fields_from_data = static function ( $data_source ) {
			...
			if ( ! empty( $data_source['url'] ) && empty( $values ) ) {
				...
				$query_args   = Encoding_Helper::parse_query_string( (string) $query_string );
		...
		$locations = array_merge(
			...
			[
				...
				'filterbar_additional_fields' => [
					'read' => [
						\Tribe__Context::FUNC          => static function () use ( $get_fb_additional_fields_from_data ) {
							// Use proper URL parsing for page refreshes to handle double-encoded values.
							if ( ! empty( $_SERVER['QUERY_STRING'] ) ) {
								$query_args = Encoding_Helper::parse_query_string( $_SERVER['QUERY_STRING'] );
								return $get_fb_additional_fields_from_data( $query_args );
							}
				...
				'filterbar_date_from'         => [
					'read' => [
						\Tribe__Context::QUERY_VAR     => [ 'tribe_datefrom' ],
						\Tribe__Context::REQUEST_VAR   => [ 'tribe_datefrom' ],
						\Tribe__Context::LOCATION_FUNC => [ 'view_data', $get_fb_val_from_view_data( 'datefrom' ) ],
					],
				],
				'filterbar_date_to'           => [
					'read' => [
						\Tribe__Context::QUERY_VAR     => [ 'tribe_dateto' ],
						\Tribe__Context::REQUEST_VAR   => [ 'tribe_dateto' ],
						\Tribe__Context::LOCATION_FUNC => [ 'view_data', $get_fb_val_from_view_data( 'dateto' ) ],
					],
				],
			],
		);
