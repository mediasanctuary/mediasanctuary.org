...
use TEC\Filter_Bar\Encoding_Helper;
 ...
 * @since 5.6.1
 *
 * @package Tribe\Events\Filterbar
 */
class Tribe__Events__Filterbar__Filter {
	...
	/**
	 * The free logic for the filter.
	 *
	 * @var string The free logic for the filter.
	 */
	public $free;
	...
	/**
	 * @var string The join clause for the filter.
	 */
	public $joinClause = '';

	/**
	 * @var string The where clause for the filter.
	 */
	public $whereClause = '';
	...
	public function __construct( $name, $slug ) {
		...
		// Initialize integrations before setting up the filter.
		$this->settings();

		$this->priority       = $this->get_priority();
		$this->isActiveFilter = $this->is_active();
		$this->currentValue   = array_map(
			[ Encoding_Helper::class, 'decode_from_url' ],
			(array) $this->get_submitted_value()
		);
	...
	public function addHooks() {
		if ( $this->is_filtering() ) {
			add_action( 'tribe_events_filter_view_do_display_filters', [ $this, 'displayFilter' ], $this->priority );
			add_action( 'tribe_events_pre_get_posts', [ $this, 'addQueryArgs' ], 10 );
			// Filter repository (ORM) queries too.
			add_action( 'tribe_repository_events_query', [ $this, 'addQueryArgs' ], 10 );
		}
		add_filter( 'tribe_events_all_filters_array', [ $this, 'allFiltersArray' ], 10, 1 );
	}

	/**
	 * Check if the filter is filtering.
	 *
	 * @return bool
	 */
	...
	public function addQueryArgs( $query ) {
		...
		if ( isset( $_REQUEST[ 'tribe_' . $this->slug ] ) && $_REQUEST[ 'tribe_' . $this->slug ] != '' ) {
			if ( ! empty( $this->joinClause ) ) {
				add_filter( 'posts_join', [ $this, 'addQueryJoin' ], 11, 2 );
			}
			if ( ! empty( $this->whereClause ) ) {
				add_filter( 'posts_where', [ $this, 'addQueryWhere' ], 11, 2 );
			}
	...
	public function addQueryWhere( $posts_where, $query ) {
		...
		remove_filter( 'posts_where', [ $this, 'addQueryWhere' ], 11, 2 );
		return $posts_where;
	}
	...
	public function addQueryJoin( $posts_join, $query ) {
		...
		remove_filter( 'posts_join', [ $this, 'addQueryJoin' ], 11, 2 );
		return $posts_join;
	}
	 ...
	 *
	 * @param array $values   The values to setup.
	 * @param bool  $dropdown Whether we're setting up a dropdown.
	 *
	 * @return array
	 */
	public function setup_dropdown_options( $values, $dropdown = false ) {
		...
		foreach ( $values as $value ) {
			...
			$options[] = [
				...
				'value' => Encoding_Helper::decode_from_url( $value['value'] ),
			];
	 ...
	 *
	 * @param array  $values         The values to setup.
	 * @param string $select_filters The select filters to setup.
	 ...
	 *
	 * @since 5.6.1 Broke out the individual filter type renders into their own methods.
	 *
	 * @return void
	 */
	public function displayFilter() {
		...
		$classes = [
			'tribe_events_filter_item' => true,
			'closed'                   => tribe_get_option( 'events_filters_layout', 'vertical' ) === 'horizontal',
			'active'                   => ! empty( $this->currentValue ),
		];
		?>
		<fieldset
			<?php tec_classes( $classes ); ?>
			id="tribe_events_filter_item_<?php echo esc_attr( $this->slug ); ?>"
		>
			<?php
			switch ( $this->type ) {
				case 'select':
					$this->do_select( $values );
					break;
				// Option for Select2 Dropdown.
				case 'multiselect':
					$this->do_multiselect( $values );
					break;
				case 'checkbox':
					$this->do_checkbox( $values );
					break;
				case 'radio':
					$this->do_radio( $values );
					break;
				case 'range':
					$this->do_range( $values );
					break;
			}
			?>
		</fieldset>
	 ...
	 * @return void
	 */
	protected function do_select( $values ) {
		// It's possible for multiple values to be specified in array form, but we can only use one of those for the select filter.
		...
		$section_title = stripslashes( $this->title );
		...
		<legend class="tribe-events-filters-legend">
			<button class="tribe-events-filters-group-heading" type="button" aria-expanded="true" aria-controls="tribe-filter-<?php echo esc_attr( $section_slug ); ?>">
				<?php echo esc_html( $section_title ); ?><span class="horizontal-drop-indicator"></span>
				<span class="tribe-filter-status"><?php echo esc_html( str_replace( '&nbsp;', '', $selected_vals ) ); ?></span>
			</button>
		</legend>
	 ...
	 * @return void
	 */
	protected function do_multiselect( $values ) {
		// Setup options in Tribe Dropdown format.
		...
		<legend class="tribe-events-filters-legend">
			<button class="tribe-events-filters-group-heading" type="button" aria-expanded="false" aria-controls="tribe-filter-<?php echo esc_attr( $section_slug ); ?>">
				<?php echo esc_html( $section_title ); ?><span class="horizontal-drop-indicator"></span>
				<span class="tribe-filter-status"><?php echo esc_html( $selected_vals ); ?></span>
			</button>
		</legend>
	...
	/**
	 * Output the checkbox filter.
	 *
	 * @param array $values The values of the filter.
	 *
	 * @return void
	 */
	protected function do_checkbox( $values ) {
		...
		?>
		<div role="group" id="<?php echo esc_attr( $section_id ); ?>" aria-label="<?php echo esc_attr( $section_id ); ?>">
			<legend class="tribe-events-filters-legend">
				<button class="tribe-events-filters-group-heading" type="button" aria-expanded="false" aria-controls="tribe-filter-<?php echo esc_attr( $section_slug ); ?>">
			...
			<div class="tribe-events-filter-group tribe-events-filter-checkboxes" id="tribe-filter-<?php echo esc_attr( $section_slug ); ?>">
				<ul>
					...
					foreach ( $values as $option ) {
						...
						echo '<li' . tec_classes( $classes ) . '>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped, StellarWP.XSS.EscapeOutput
						echo '<input type="checkbox" id="'
							. esc_attr( str_replace( ' ', '-', stripslashes( strtolower( $option['name'] ) ) ) )
							. '" value="' . esc_attr( $option['value'] )
							. '" '
							. checked( $this->is_selected( trim( $option['value'] ) ), true, false )
							. ' name="'
							. esc_attr( 'tribe_' . $this->slug . '[]' )
							. '" ' . $data // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped, StellarWP.XSS.EscapeOutput.OutputNotEscaped
							. ' aria-labelledby="'
							. esc_attr( str_replace( ' ', '-', stripslashes( strtolower( $option['name'] ) ) ) )
							. ' '
							. esc_attr( $section_id ) . '" />';
						echo '<label for="' . esc_attr( str_replace( ' ', '-', stripslashes( strtolower( $option['name'] ) ) ) ) . '">';
	...
	/**
	 * Output the radio filter.
	 *
	 * @param array $values The values of the filter.
	 *
	 * @return void
	 */
	protected function do_radio( $values ) {
		if ( ! isset( $this->currentValue ) ) {
			$current_value = '';
		} else {
		...
		if ( ! empty( $current_value ) ) {
			$selected_option = array_filter(
				$values,
				function ( $value ) use ( $current_value ) {
		...
		<ul>
			...
			foreach ( $values as $option ) {
				...
				// Output option to screen.
				echo '<li ' . tec_classes( $classes ) . '>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped, StellarWP.XSS.EscapeOutput.OutputNotEscaped
				echo '<input type="radio" id="' . esc_attr( str_replace( ' ', '-', stripslashes( strtolower( $option['name'] ) ) ) ) . '" value="' . esc_attr( $option['value'] ) . '" ' . checked( trim( $option['value'] ), $current_value, false ) . ' name="' . esc_attr( 'tribe_' . $this->slug ) . '" ' . $data . ' />'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped, StellarWP.XSS.EscapeOutput.OutputNotEscaped
				echo '<label for="' . esc_attr( str_replace( ' ', '-', stripslashes( strtolower( $option['name'] ) ) ) ) . '">';
				echo '<span>' . esc_html( stripslashes( $option['name'] ) ) . '</span>';
				echo '</label>';
				echo '</li>';
	 ...
	 * @param int    $default_value The default value to return.
	 *
	 * @return int
	 */
	protected function to_int( $value = '', $direction = 'down', $default_value = 0 ) {
		if ( ! is_numeric( $value ) ) {
	...
	/**
	 * Output the range filter.
	 *
	 * @param array $values The values of the filter.
	 *
	 * @return void
	 */
	protected function do_range( $values ) {
		...
		?>
			<legend class="tribe-events-filters-legend">
				<button class="tribe-events-filters-group-heading" type="button" aria-expanded="false" aria-controls="tribe-filter-<?php echo esc_attr( $section_slug ); ?>">
					<?php echo esc_html( $section_title ); ?><span class="horizontal-drop-indicator"></span>
					<span class="tribe-filter-status"><?php esc_html( $display_value ); ?></span>
				</button>
			</legend>
			<div class="tribe-events-filter-group tribe-events-filter-range" id="tribe-filter-<?php echo esc_attr( $section_slug ); ?>">
			...
			<script>
				jQuery(function ($) {
					$( "#<?php echo esc_attr( 'tribe_events_filter_' . $this->slug . '_slider' ); ?>" ).slider({
						range: true,
						min: <?php echo esc_attr( $min_value ); ?>,
						max: <?php echo esc_attr( $max_value ); ?>,
						values: [ <?php echo esc_attr( $this->to_int( $current['min'] ) ); ?>, <?php echo esc_attr( $this->to_int( $current['max'], 'up' ) ); ?> ],
						slide: function ( event, ui ) {
							<?php if ( $reverse_position ) { ?>
								$( "#<?php echo 'tribe_events_filter_' . esc_attr( $this->slug ); ?>_display" ).text( ui.values[ 0 ] + "<?php echo esc_attr( $currency_symbol ); ?>" + "-" + ui.values[ 1 ] + "<?php echo esc_attr( $currency_symbol ); ?>" );
							<?php } else { ?>
								$( "#<?php echo 'tribe_events_filter_' . esc_attr( $this->slug ); ?>_display" ).text( "<?php echo esc_attr( $currency_symbol ); ?>" + ui.values[ 0 ] + "-<?php echo esc_attr( $currency_symbol ); ?>" + ui.values[ 1 ] );
							<?php } ?>

							if( ui.values[ 0 ] === <?php echo esc_attr( $min_value ); ?> && <?php echo esc_attr( $max_value ); ?> === ui.values[ 1 ] ) {
								$( "#<?php echo 'tribe_events_filter_' . esc_attr( $this->slug ); ?>" ).val('');
							} else {
								$( "#<?php echo 'tribe_events_filter_' . esc_attr( $this->slug ); ?>" ).val( ui.values[ 0 ] + "-" + ui.values[ 1 ] );
							}
						}
					});
				<?php if ( $reverse_position ) { ?>
					$( "#<?php echo 'tribe_events_filter_' . esc_attr( $this->slug ); ?>_display" ).text( $( "#<?php echo 'tribe_events_filter_' . esc_attr( $this->slug ) . '_slider'; ?>" ).slider( "values", 0 ) + "<?php echo esc_attr( $currency_symbol ); ?>" + "-" + $( "#<?php echo 'tribe_events_filter_' . esc_attr( $this->slug ) . '_slider'; ?>" ).slider( "values", 1 ) + "<?php echo esc_attr( $currency_symbol ); ?>" );
				<?php } else { ?>
					$( "#<?php echo 'tribe_events_filter_' . esc_attr( $this->slug ); ?>_display" ).text( "<?php echo esc_attr( $currency_symbol ); ?>" + $( "#<?php echo 'tribe_events_filter_' . esc_attr( $this->slug ) . '_slider'; ?>" ).slider( "values", 0 ) + "-<?php echo esc_attr( $currency_symbol ); ?>" + $( "#<?php echo 'tribe_events_filter_' . esc_attr( $this->slug ) . '_slider'; ?>" ).slider( "values", 1 ) );
				<?php } ?>
				});
			</script>
		<?php
	 ...
	 *
	 * @param mixed $option The option to test.
	 *
	 * @return bool
	 */
	 ...
	 * Get the admin form.
	 *
	 * @todo: where is "adminForm" defined?
	 *
	 * @return string The admin form.
	 */
	public function get_admin_form() {
		// phpcs:disable WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		return $this->adminForm ?? $this->get_title_field();
	}
	...
	protected function get_title_field() {
		return sprintf(
			...
			sprintf(
				/* Translators: %1$s is the field name, %2$s is the title value. */
	 ...
	 *
	 * @return string The field type options.
	 */
	protected function get_multichoice_type_field() {
		$name  = $this->get_admin_field_name( 'type' );
		$field = sprintf(
			/* Translators: %1$s is the dropdown option, %2$s is the checkboxes option, %3$s is the multi-select option */
			__( 'Type: %1$s %2$s %3$s', 'tribe-events-filter-view' ),
			sprintf(
				/* Translators: %1$s is the radio button name, %2$s is the checked state, %3$s is the label */
				'<label><input type="radio" name="%s" value="select" %s /> %s</label>',
				$name,
			...
			sprintf(
				/* Translators: %1$s is the radio button name, %2$s is the checked state, %3$s is the label */
				'<label><input type="radio" name="%s" value="checkbox" %s /> %s</label>',
				$name,
			...
			sprintf(
				/* Translators: %1$s is the radio button name, %2$s is the checked state, %3$s is the label */
				'<label><input type="radio" name="%s" value="multiselect" %s /> %s</label>',
				$name,
	...
	/**
	 * Get the admin field name.
	 *
	 * @param string $name The name of the field.
	 *
	 * @return string The admin field name.
	 */
	...
	public function get_title() {
		...
		$title                  = $current_active_filters[ $this->slug ]['title'] ?? $this->name;
	...
	protected function get_type() {
		$current_active_filters = Tribe__Events__Filterbar__View::instance()->get_filter_settings();
		$type                   = $current_active_filters[ $this->slug ]['type'] ?? $this->type;
	...
	protected function is_active() {
		...
		/**
		 * Filters whether the filter is active.
		 *
	 ...
	 *
	 * @return array The values of the filter.
	 */
	...
	public function get_name_field() {
		...
		// Add a trailing `[]` to the field name if the type requires it.
	...
	public function get_fields_data_by_type() {
		...
		switch ( $this->type ) {
			...
			case 'range':
				...
				break;
			case 'date':
				$formatted_values[] = $this->get_date_format();
				break;
		}
	...
	/**
	 * Get the Value Formatted for a Calendar Template.
	 *
	 * @since 5.6.0
	 *
	...
	public function get_date_format() {
		// For calendar type, we don't need predefined values.
		// phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		$current_value = is_array( $this->currentValue ) ? reset( $this->currentValue ) : $this->currentValue;
	...
	public function get_dropdown_format() {
		...
		$current_values = array_map(
			static function ( $current ) {
				return html_entity_decode( Encoding_Helper::decode_from_url( (string) $current ), ENT_QUOTES );
			},
		...
		foreach ( $this->values as $value ) {
			...
			$options[] = [
				...
				'value'    => $value,
				'selected' => in_array( html_entity_decode( Encoding_Helper::decode_from_url( (string) $value ), ENT_QUOTES ), $current_values, true ),
			];
		}
		...
		if ( is_string( $var ) ) {
			$var = Encoding_Helper::decode_from_url( $var );
		}
	...
	public function get_range_format() {
		...
		$min      = $selected['min'] ?? false;
		$max      = $selected['max'] ?? false;
		...
		$selected = is_array( $selected ) ? implode( '-', $selected ) : (string) $selected;

		return [
			'type'  => $this->type,
			'label' => $label,
			'value' => Encoding_Helper::decode_from_url( $selected ),
			'id'    => $this->get_formatted_id_from_name( $this->slug ),
			'name'  => $this->get_name_field(),
			'min'   => $this->values['min'],
			'max'   => $this->values['max'],
	...
	public function get_is_checked( $value ) {
		...
		if ( $this->currentValue ) {
			$normalized_current = array_map( [ $this, 'normalize_compare_value' ], (array) $this->currentValue );

			return in_array( $this->normalize_compare_value( $value ), $normalized_current, true );
		}
	...
	/**
	 * Normalizes a value to the canonical form used to compare an option/stored value
	 * against the current filter value.
	 *
	 * Additional field values may arrive URL-encoded (e.g. `%2526`) and are stored by
	 * Events Calendar PRO HTML-encoded (e.g. `&amp;`), while the current filter value is
	 * decoded (e.g. `&`). Decoding both sides the same way lets them match.
	 *
	 * @since 5.6.6
	 *
	 * @param mixed $value The value to normalize.
	 *
	 * @return string The URL- and HTML-entity-decoded value.
	 */
	protected function normalize_compare_value( $value ): string {
		if ( ! is_scalar( $value ) ) {
			return '';
		}

		return html_entity_decode( Encoding_Helper::decode_from_url( (string) $value ), ENT_QUOTES );
	}
	...
	public function get_current_value_for_display() {
		...
		if (
			...
			/*
			 * Encoding-tolerant direct match: additional field option values can contain special
			 * characters (commas, `&` stored as `&amp;`). Match the selected value(s) to an option
			 * on their normalized form and return the first selected label; fall through to the
			 * legacy comma-delimited-ID grouping below only when no direct match is found.
			 */
			$normalized_options = [];
			foreach ( $plucked as $option_value => $option_name ) {
				$normalized_options[ $this->normalize_compare_value( $option_value ) ] = $option_name;
			}

			foreach ( (array) $this->currentValue as $current_value ) {
				$normalized_current = $this->normalize_compare_value( $current_value );

				if ( isset( $normalized_options[ $normalized_current ] ) ) {
					return $normalized_options[ $normalized_current ];
				}
			}
		} elseif (
			'radio' === $this->type
			...
			$current = $this->normalize_compare_value( current( (array) $this->currentValue ) );

			foreach ( $display_current as $option_value => $option_name ) {
				if ( $this->normalize_compare_value( $option_value ) === $current ) {
					return $option_name;
				}
			}
			...
			// For date type, format the date according to the site's date format.
			$date_value = current( $select_filters );
			if ( ! empty( $date_value ) ) {
				// Use WordPress date formatting.
				$date_format = get_option( 'date_format', 'F j, Y' );

				return date_i18n( $date_format, strtotime( $date_value ) );
			}
	...
	protected function stringify_value( $value ) {
		return Encoding_Helper::decode_from_url( str_replace( ',', '-', Arr::to_list( array_values( (array) $value ) ) ) );
	}
